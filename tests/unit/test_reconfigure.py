...
def test_reconfigure_missing_from_cluster_returns_1() -> None:
    """A guest in the state log but absent from the cluster can't be
    reconfigured (nothing to target) -> returns 1."""
    with patch("pmx.reconfigure.load") as mock_load, \
         patch("pmx.reconfigure.find_by_name") as mock_find, \
         patch("pmx.reconfigure.query_cluster") as mock_query, \
        ...
        mock_query.return_value = {}  # not on any node

        result = run("testhost")

        error_calls = [c for c in mock_echo.call_args_list if "cannot reconfigure" in str(c)]
        assert len(error_calls) > 0
...
def test_reconfigure_state_found_builds_correct_extra_vars() -> None:
    """Test that reconfigure with found state builds correct extra_vars and
    targets the node the cluster reports (not the hardcoded default_node)."""
    with patch("pmx.reconfigure.load") as mock_load, \
         patch("pmx.reconfigure.find_by_name") as mock_find, \
         patch("pmx.reconfigure.query_cluster") as mock_query, \
        ...
        # Guest lives on excelsior, NOT the default node pve01.
        mock_query.return_value = {"testhost": (101, "vm", "excelsior")}
        ...
        # Node comes from cluster discovery, not cfg.default_node.
...
def test_reconfigure_not_domain_joined_skips_password() -> None:
    ...
    with patch("pmx.reconfigure.load") as mock_load, \
        ...
        mock_query.return_value = {"testhost": (101, "lxc", "kelvin")}
        mock_playbook.return_value = 0
        ...
        # Verify playbook was called, targeting the discovered node.
        mock_playbook.assert_called_once()
        assert mock_playbook.call_args[0][1]["target_node"] == "kelvin"
        assert result == 0
